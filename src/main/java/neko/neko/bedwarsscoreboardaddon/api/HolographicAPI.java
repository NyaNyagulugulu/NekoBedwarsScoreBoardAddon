package neko.neko.bedwarsscoreboardaddon.api;
...
import neko.neko.bedwarsscoreboardaddon.utils.Utils;
				...
				if (player != null && player.isOnline()) {
					...
					if (loc.getWorld().getName().equals(location.getWorld().getName()) && loc.distanceSquared(location) < Math.pow(64, 2) && player != null && player.isOnline() && players.contains(uuid)) {
						...
						if (BedwarsRel.getInstance().getCurrentVersion().startsWith("v1_8")) {
							...
							// Initialize serializers if they are null
							...
							// Check if serializers are properly initialized before proceeding
							if (stringserializer != null && booleanserializer != null) {
								...
								if (title != null) {
									watchableObjects.add(new WrappedWatchableObject(new WrappedDataWatcher.WrappedDataWatcherObject(2, stringserializer), title));
									watchableObjects.add(new WrappedWatchableObject(new WrappedDataWatcher.WrappedDataWatcherObject(3, booleanserializer), true));
								} else {
									watchableObjects.add(new WrappedWatchableObject(new WrappedDataWatcher.WrappedDataWatcherObject(2, stringserializer), ""));
								...
								// Fallback to v1_8 method if serializers can't be initialized
								packet.getWatchableCollectionModifier().write(0, Arrays.asList(new WrappedWatchableObject(2, title)));
							}
						}
